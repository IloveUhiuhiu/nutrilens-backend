...
def upload_image_to_cloudinary(uploaded_file, public_id=None, folder=None):
    """Upload file anh len Cloudinary va tra secure_url."""
    ...
    if not settings.CLOUDINARY_API_KEY or not settings.CLOUDINARY_API_SECRET:
        raise CloudinaryUploadError(
            "Cloudinary upload requires CLOUDINARY_API_KEY and CLOUDINARY_API_SECRET.",
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
        )
